...
async function fetchVariationsForUserActivities(supabase: any, userId: string, activityIds: string[]): Promise<Map<string, VariationRow>> {
  ...
  for (let i = 0; i < activityIds.length; i += pageSize) {
    ...
    const { data, error } = await supabase
      ...
      .select('user_id,activity_id,pace_cv,heart_rate_cv')
      .eq('user_id', userId)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Prepare var (kept for type-checking)
    let activities: ActivityRow[] = []
    // PAUSED: stop all processing immediately (temporary kill-switch)
    return new Response(
      JSON.stringify({ success: true, processed: 0, message: 'classification paused by admin' }),
...
// Buscar CVs oficiais da variation_analysis
const varRow = variationMap.get(String(a.activity_id))
const pace_cv = varRow?.pace_cv ?? null
const hr_cv = varRow?.heart_rate_cv ?? null
