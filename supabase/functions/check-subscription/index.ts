...
serve(async (req) => {
  ...
  try {
    ...
    if (hasActiveSub) {
      ...
      const annualPayment = oneTimePayments.data.find(payment => 
        payment.status === 'succeeded' && 
        payment.created > (Date.now() / 1000) - (365 * 24 * 60 * 60) &&
        payment.amount === 15480 // R$ 154,80 in cents
      );
      ...
      if (annualPayment) {
        ...
        subscriptionTier = "Premium";
        subscriptionEnd = new Date(annualPayment.created * 1000 + (365 * 24 * 60 * 60 * 1000)).toISOString();
        logStep("Valid annual payment found", { paymentId: annualPayment.id, endDate: subscriptionEnd });
      }
    }
