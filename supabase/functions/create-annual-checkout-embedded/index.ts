  ...
  try {
    ...
    if (stripeMode === "live") {
      priceId = firstDefined(
        Deno.env.get("STRIPE_PRICE_ID_ANNUAL_LIVE"),
        Deno.env.get("STRIPE_ANNUAL_PRICE_ID_LIVE"),
        Deno.env.get("STRIPE_PRICE_ID_ANNUAL")
      );
    } else if (stripeMode === "test") {
      priceId = firstDefined(
        Deno.env.get("STRIPE_PRICE_ID_ANNUAL_TEST"),
    ...
    if (!priceId) {
      ...
      priceId = firstDefined(
        stripeMode === "live"
          ? map.get("stripe_price_id_annual_live") || map.get("stripe_price_id_annual") || map.get("stripe_annual_price_id")
          : stripeMode === "test"
          ? map.get("stripe_price_id_annual_test") || map.get("stripe_price_id_annual") || map.get("stripe_annual_price_id")
          : map.get("stripe_price_id_annual") || map.get("stripe_annual_price_id")
      ) as string | undefined;
    }

    if (!priceId) throw new Error("Annual Stripe price ID not configured");
