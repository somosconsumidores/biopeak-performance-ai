...
serve(async (req) => {
  ...
  try {
    ...
    // Default to yesterday if no target_date provided
    ...
    console.log(`✅ Batch complete: ${successCount} success, ${errorCount} errors in ${duration}s`);
...
async function calculateAndSaveFitnessScore(supabase: any, userId: string, targetDate: string): Promise<void> {
  ...
  // Calculate ATL (7-day) and CTL (42-day)
