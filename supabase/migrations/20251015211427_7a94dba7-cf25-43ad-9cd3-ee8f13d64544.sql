-- Fix weekly_summary_stats_v2 - Add explicit casts for ALL double precision fields
DROP FUNCTION IF EXISTS public.weekly_summary_stats_v2(date, date, date, date);
...
BEGIN
  ...
  WITH current_week AS (
    SELECT 
      ...
      ROUND((SUM(aa.total_distance_meters) / 1000.0)::numeric, 2) as total_km,
      ROUND((SUM(aa.total_time_minutes) / 60.0)::numeric, 2) as total_hours,
      ROUND(SUM(aa.active_kilocalories)::numeric, 0) as calories,
      ROUND((AVG(NULLIF(aa.pace_min_per_km, 0)) FILTER (WHERE aa.pace_min_per_km > 0 AND aa.pace_min_per_km < 15))::numeric, 2) as avg_pace,
      ...
      ROUND(SUM(COALESCE(aa.total_elevation_gain_in_meters, 0))::numeric, 0) as elevation,
      ROUND((MAX(aa.total_distance_meters) / 1000.0)::numeric, 2) as longest_dist,
      ROUND((MAX(aa.total_time_minutes) / 60.0)::numeric, 2) as longest_dur,
      ROUND((MIN(NULLIF(aa.pace_min_per_km, 0)) FILTER (WHERE aa.pace_min_per_km > 0 AND aa.pace_min_per_km < 15))::numeric, 2) as best_pace
  ...
  previous_week AS (
    SELECT 
      ...
      ROUND((SUM(aa.total_distance_meters) / 1000.0)::numeric, 2) as prev_total_km
    FROM public.all_activities aa
  ...
  SELECT 
    ...
    CASE 
      WHEN pw.prev_total_km > 0 THEN 
        ROUND((((cw.total_km - pw.prev_total_km) / pw.prev_total_km) * 100)::numeric, 1)
      ELSE NULL
    ...
    ROUND((cw.total_km / cw.activities_count)::numeric, 1) as avg_km_per_activity
