-- Create variation_analysis table with RLS and policies, trigger, calculator function, triggers on detail tables, and a light backfill
...
-- 3) Policies (idempotent creation)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 5) Calculator function (SECURITY DEFINER)
...
BEGIN
  -- Compute from Garmin details if requested
  IF lower(p_activity_source) = 'garmin' AND to_regclass('public.garmin_activity_details') IS NOT NULL THEN
    SELECT
  ...
  ELSIF lower(p_activity_source) = 'strava' AND to_regclass('public.strava_activity_details') IS NOT NULL THEN
    -- Best-effort: try common columns if available
    EXECUTE $$
      SELECT
        COUNT(*) FILTER (WHERE heart_rate IS NOT NULL OR speed_m_s IS NOT NULL OR velocity_smooth IS NOT NULL) AS cnt,
        AVG(heart_rate)::numeric AS hr_avg,
        STDDEV_POP(heart_rate)::numeric AS hr_sd,
        AVG(CASE 
              WHEN speed_m_s IS NOT NULL AND speed_m_s > 0 THEN (1000.0/speed_m_s)/60.0
              WHEN velocity_smooth IS NOT NULL AND velocity_smooth > 0 THEN (1000.0/velocity_smooth)/60.0
              ELSE NULL
            END)::numeric AS pace_avg,
        STDDEV_POP(CASE 
              WHEN speed_m_s IS NOT NULL AND speed_m_s > 0 THEN (1000.0/speed_m_s)/60.0
              WHEN velocity_smooth IS NOT NULL AND velocity_smooth > 0 THEN (1000.0/velocity_smooth)/60.0
              ELSE NULL
            END)::numeric AS pace_sd
      FROM public.strava_activity_details
      WHERE user_id = $1 AND strava_activity_id::text = $2
    $$
    INTO v_count, v_hr_avg, v_hr_sd, v_pace_avg, v_pace_sd
    USING p_user_id, p_activity_id;
  ELSIF lower(p_activity_source) = 'strava_gpx' AND to_regclass('public.strava_gpx_activity_details') IS NOT NULL THEN
    EXECUTE $$
      SELECT
        COUNT(*) FILTER (WHERE heart_rate IS NOT NULL OR speed_m_s IS NOT NULL) AS cnt,
        AVG(heart_rate)::numeric AS hr_avg,
        STDDEV_POP(heart_rate)::numeric AS hr_sd,
        AVG(CASE WHEN speed_m_s IS NOT NULL AND speed_m_s > 0 THEN (1000.0/speed_m_s)/60.0 ELSE NULL END)::numeric AS pace_avg,
        STDDEV_POP(CASE WHEN speed_m_s IS NOT NULL AND speed_m_s > 0 THEN (1000.0/speed_m_s)/60.0 ELSE NULL END)::numeric AS pace_sd
      FROM public.strava_gpx_activity_details
      WHERE user_id = $1 AND activity_id = $2
    $$
    INTO v_count, v_hr_avg, v_hr_sd, v_pace_avg, v_pace_sd
    USING p_user_id, p_activity_id;
  ELSIF lower(p_activity_source) = 'polar' AND to_regclass('public.polar_activity_details') IS NOT NULL THEN
    EXECUTE $$
      SELECT
        COUNT(*) FILTER (WHERE heart_rate IS NOT NULL OR speed_m_s IS NOT NULL) AS cnt,
        AVG(heart_rate)::numeric AS hr_avg,
    ...
    INTO v_count, v_hr_avg, v_hr_sd, v_pace_avg, v_pace_sd
    USING p_user_id, p_activity_id;
  ELSIF lower(p_activity_source) = 'zepp_gpx' AND to_regclass('public.zepp_gpx_activity_details') IS NOT NULL THEN
    EXECUTE $$
      SELECT
        COUNT(*) FILTER (WHERE heart_rate IS NOT NULL OR speed_m_s IS NOT NULL) AS cnt,
        AVG(heart_rate)::numeric AS hr_avg,
    ...
    USING p_user_id, p_activity_id;
  ELSE
    -- Unknown source or table missing: just exit
    RETURN;
  END IF;

  IF COALESCE(v_pace_avg,0) > 0 THEN
    v_pace_cv := NULLIF(v_pace_sd,0) / v_pace_avg; -- SD/mean for pace
  ELSE
    v_pace_cv := NULL;
  END IF;
  IF COALESCE(v_hr_avg,0) > 0 THEN
    v_hr_cv := NULLIF(v_hr_sd,0) / v_hr_avg; -- SD/mean for HR
  ELSE
    v_hr_cv := NULL;
  END IF;
...
-- 6) Single trigger function to recalc on details insert/update
...
AS $$
BEGIN
  IF TG_TABLE_NAME = 'garmin_activity_details' THEN
    PERFORM public.calculate_variation_analysis(NEW.user_id, 'garmin', NEW.activity_id);
  ELSIF TG_TABLE_NAME = 'strava_activity_details' THEN
    PERFORM public.calculate_variation_analysis(NEW.user_id, 'strava', COALESCE(NEW.strava_activity_id::text, NEW.activity_id::text));
  ELSIF TG_TABLE_NAME = 'strava_gpx_activity_details' THEN
    PERFORM public.calculate_variation_analysis(NEW.user_id, 'strava_gpx', NEW.activity_id);
  ELSIF TG_TABLE_NAME = 'polar_activity_details' THEN
    PERFORM public.calculate_variation_analysis(NEW.user_id, 'polar', NEW.activity_id);
  ELSIF TG_TABLE_NAME = 'zepp_gpx_activity_details' THEN
...
-- 7) Attach triggers conditionally if tables exist
DO $$
BEGIN
  ...
  IF to_regclass('public.strava_activity_details') IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_strava_details_variation_recalc') THEN
      CREATE TRIGGER trg_strava_details_variation_recalc
      AFTER INSERT OR UPDATE ON public.strava_activity_details
      FOR EACH ROW EXECUTE FUNCTION public.trg_variation_analysis_recalc();
    END IF;
  END IF;

  IF to_regclass('public.strava_gpx_activity_details') IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_strava_gpx_details_variation_recalc') THEN
      CREATE TRIGGER trg_strava_gpx_details_variation_recalc
      AFTER INSERT OR UPDATE ON public.strava_gpx_activity_details
      FOR EACH ROW EXECUTE FUNCTION public.trg_variation_analysis_recalc();
    END IF;
  END IF;

  IF to_regclass('public.polar_activity_details') IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_polar_details_variation_recalc') THEN
      CREATE TRIGGER trg_polar_details_variation_recalc
      AFTER INSERT OR UPDATE ON public.polar_activity_details
      FOR EACH ROW EXECUTE FUNCTION public.trg_variation_analysis_recalc();
    END IF;
  END IF;

  IF to_regclass('public.zepp_gpx_activity_details') IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_zepp_gpx_details_variation_recalc') THEN
      CREATE TRIGGER trg_zepp_gpx_details_variation_recalc
      AFTER INSERT OR UPDATE ON public.zepp_gpx_activity_details
      FOR EACH ROW EXECUTE FUNCTION public.trg_variation_analysis_recalc();
    END IF;
  END IF;
END $$;

-- 8) Light backfill for recent Garmin details (last 90 days)
DO $$
DECLARE r record; 
BEGIN
  IF to_regclass('public.garmin_activity_details') IS NOT NULL THEN
