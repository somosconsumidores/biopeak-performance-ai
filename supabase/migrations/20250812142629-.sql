-- Create tables for Strava GPX import and storage policies for GPX uploads
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_strava_gpx_activities_user_activity_id 
  ON public.strava_gpx_activities(user_id, activity_id);
CREATE INDEX IF NOT EXISTS idx_strava_gpx_activities_user_start_time 
...
CREATE INDEX IF NOT EXISTS idx_strava_gpx_activity_details_user_activity 
  ON public.strava_gpx_activity_details(user_id, activity_id);
CREATE INDEX IF NOT EXISTS idx_strava_gpx_activity_details_timestamp 
...
-- 3) Storage policies for GPX uploads
-- Allow authenticated users to manage files only inside their own folder: <user_id>/...
DO $$
BEGIN
  IF NOT EXISTS (
