...
DECLARE
  ...
  v_pace double precision[] := ARRAY[]::double precision[]; -- min/km
...
BEGIN
  ...
  IF _activity_source = 'garmin' THEN
    -- Prefer samples->'metricDescriptors' datasets when available; fallback to per-sample columns
  ...
  ELSIF _activity_source = 'strava' THEN
    -- Strava details table name; if absent, this SELECT returns no rows
    SELECT array_agg(hr)::double precision[], array_agg(pmin)::double precision[]
    INTO v_hr, v_pace
    FROM (
      SELECT sad.user_id,
             sad.strava_activity_id::text AS activity_id,
             NULLIF((sad.samples->>'heartrate')::numeric, NULL)::double precision AS hr,
             CASE
  ...
  ELSIF _activity_source = 'strava_gpx' THEN
    ...
    FROM (
      SELECT sgd.user_id,
             sgd.activity_id,
             sgd.heart_rate::double precision AS hr,
  ...
  ELSIF _activity_source = 'polar' THEN
    ...
    FROM (
      SELECT pad.user_id,
             pad.activity_id,
             pad.heart_rate_bpm::double precision AS hr,
  ...
  ELSIF _activity_source = 'zepp_gpx' THEN
    ...
    FROM (
      SELECT zgd.user_id,
             zgd.activity_id,
             zgd.heart_rate::double precision AS hr,
...
-- 3) Triggers on detail tables (create if tables exist)
DO $$
BEGIN
  -- Garmin
...
-- 4) Optional backfill for recent activities (best effort, only where details exist)
DO $$
DECLARE
  r RECORD;
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='strava_activity_details') THEN
    FOR r IN (
      SELECT DISTINCT sad.user_id, sad.strava_activity_id::text AS activity_id
      FROM public.strava_activity_details sad
      LEFT JOIN public.variation_analysis va
      ...
      PERFORM public.calculate_variation_analysis(r.user_id, 'strava', r.activity_id);
    END LOOP;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='strava_gpx_activity_details') THEN
    FOR r IN (
      SELECT DISTINCT sgd.user_id, sgd.activity_id
      FROM public.strava_gpx_activity_details sgd
      LEFT JOIN public.variation_analysis va
        ON va.user_id = sgd.user_id AND va.activity_id = sgd.activity_id AND va.activity_source = 'strava_gpx'
      WHERE va.id IS NULL
      LIMIT 500
    ) LOOP
      PERFORM public.calculate_variation_analysis(r.user_id, 'strava_gpx', r.activity_id);
    END LOOP;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='polar_activity_details') THEN
    FOR r IN (
      SELECT DISTINCT pad.user_id, pad.activity_id
      FROM public.polar_activity_details pad
      LEFT JOIN public.variation_analysis va
      ...
      PERFORM public.calculate_variation_analysis(r.user_id, 'polar', r.activity_id);
    END LOOP;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='zepp_gpx_activity_details') THEN
    FOR r IN (
      SELECT DISTINCT zgd.user_id, zgd.activity_id
      FROM public.zepp_gpx_activity_details zgd
      LEFT JOIN public.variation_analysis va
        ON va.user_id = zgd.user_id AND va.activity_id = zgd.activity_id AND va.activity_source = 'zepp_gpx'
      WHERE va.id IS NULL
      LIMIT 500
    ) LOOP
      PERFORM public.calculate_variation_analysis(r.user_id, 'zepp_gpx', r.activity_id);
    END LOOP;
  END IF;
END $$;