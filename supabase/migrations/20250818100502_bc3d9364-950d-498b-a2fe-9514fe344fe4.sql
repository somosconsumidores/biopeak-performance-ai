-- Replace RPC to robustly reprocess Garmin userMetrics into garmin_vo2max
...
BEGIN
  ...
  CROSS JOIN LATERAL jsonb_array_elements(
    CASE 
      WHEN jsonb_typeof(l.payload->'userMetrics') = 'array' THEN l.payload->'userMetrics'
      WHEN jsonb_typeof(l.payload->'userMetrics') = 'object' THEN jsonb_build_array(l.payload->'userMetrics')
      ELSE '[]'::jsonb
    END
  ) AS m
