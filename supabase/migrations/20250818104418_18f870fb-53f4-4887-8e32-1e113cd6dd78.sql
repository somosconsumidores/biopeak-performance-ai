...
BEGIN
  ...
  IF NEW.average_speed IS NOT NULL AND NEW.average_speed > 0 THEN
    pace_min_km := (1000.0 / NEW.average_speed::numeric) / 60.0;
  ...
  IF pace_min_km IS NOT NULL
    ...
    NEW.vo2_max := public.calculate_vo2_max(
      NEW.type,
      pace_min_km,
      NEW.average_heartrate::int,
      NEW.max_heartrate::int
    );
  ELSE
    NEW.vo2_max := NULL;
  END IF;
...
SET vo2_max = public.calculate_vo2_max(
  ...
  CASE
    WHEN sa.average_speed IS NOT NULL AND sa.average_speed > 0
      THEN (1000.0 / sa.average_speed::numeric) / 60.0
    ...
    ELSE NULL
  END,
  sa.average_heartrate::int,
  sa.max_heartrate::int
)
