-- Corrige a função find_fastest_1km_segment para usar o campo correto distance_m
CREATE OR REPLACE FUNCTION public.find_fastest_1km_segment(p_user_id uuid, p_activity_id text, p_activity_source text DEFAULT 'garmin'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
  ...
  -- Convert to array for easier processing, using correct field name distance_m
  SELECT array_agg(value ORDER BY (value->>'distance_m')::numeric)
  INTO points_array
  FROM jsonb_array_elements(series_data) AS value
  WHERE value->>'distance_m' IS NOT NULL 
    AND (value->>'distance_m')::numeric >= 0;
