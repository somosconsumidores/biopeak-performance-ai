...
-- Atualiza a RPC para lidar com payload achatado e/ou userMetrics
...
BEGIN
  ...
  -- Coleta candidatos a partir de dois formatos possíveis:
  -- 1) payload.userMetrics (array/objeto)
  -- 2) payload achatado com userId + calendarDate no nível raiz
  ...
  CROSS JOIN LATERAL jsonb_array_elements(
    CASE
      WHEN l.payload ? 'userMetrics' AND jsonb_typeof(l.payload->'userMetrics') = 'array'
        THEN l.payload->'userMetrics'
      WHEN l.payload ? 'userMetrics' AND jsonb_typeof(l.payload->'userMetrics') = 'object'
        THEN jsonb_build_array(l.payload->'userMetrics')
      WHEN l.payload ? 'userId' AND l.payload ? 'calendarDate'
        THEN jsonb_build_array(l.payload)
      ELSE '[]'::jsonb
    END
  ) AS m
  WHERE l.webhook_type IN ('userMetrics', 'user_metrics');
  ...
  WITH aggregated AS (
    ...
    WHERE garmin_user_id IS NOT NULL
      ...
      AND (vo2_max_running IS NOT NULL OR vo2_max_cycling IS NOT NULL)
    GROUP BY 1,2
  ), upserted AS (
...
-- Permissão de execução (a função valida admin internamente)
