...
CREATE TABLE IF NOT EXISTS public.variation_analysis (
  ...
  activity_date date,
  activity_type text,
  activity_source text NOT NULL, -- 'garmin' | 'strava' | 'strava_gpx' | 'zepp_gpx' | 'polar' | 'zepp'
...
-- Service role pode gerenciar (ALL)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- Execução inicial (opcional): enfileira e processa um lote
SELECT public.enqueue_missing_variation_analysis_jobs();
SELECT public.process_variation_analysis_jobs(500);

-- Verificação rápida (opcional)
-- SELECT status, COUNT(*) FROM public.variation_analysis_jobs GROUP BY status ORDER BY status;
