-- Update calculate_variation_analysis function to support HealthKit from activity_chart_data
...
BEGIN
  ...
  CASE lower(p_activity_source)
    WHEN 'healthkit' THEN
      -- NEW: Use activity_chart_data for HealthKit activities
      WITH s AS (
        SELECT
          CASE 
            WHEN elem->>'pace_min_km' IS NOT NULL AND (elem->>'pace_min_km')::numeric > 0
              THEN (elem->>'pace_min_km')::numeric
            WHEN elem->>'speed_ms' IS NOT NULL AND (elem->>'speed_ms')::numeric > 0
              THEN 1000.0 / ((elem->>'speed_ms')::numeric * 60.0)
            ELSE NULL
          END AS pace,
          CASE 
            WHEN elem->>'heart_rate' IS NOT NULL AND (elem->>'heart_rate')::numeric > 0
              THEN (elem->>'heart_rate')::numeric
            ELSE NULL
          END AS hr
        FROM public.activity_chart_data acd
        CROSS JOIN LATERAL jsonb_array_elements(acd.series_data) elem
        WHERE acd.user_id = p_user_id
          AND acd.activity_id = p_activity_id
          AND acd.activity_source = 'healthkit'
      )
      SELECT 
    ...
    WHEN 'garmin' THEN
      WITH s AS (
        ...
        -- 2) Fallback JSONB (para compatibilidade com formatos antigos)
        SELECT
          CASE
    ...
    WHEN 'strava' THEN
      -- Já corrigido: usa colunas por amostra de public.strava_activity_details
      WITH s AS (
        SELECT
          CASE
    ...
    WHEN 'strava_gpx' THEN
      -- Corrigido: usa colunas por amostra da tabela public.strava_gpx_activity_details
      WITH s AS (
        SELECT
          CASE
            WHEN d.speed_meters_per_second IS NOT NULL AND d.speed_meters_per_second > 0
    ...
    WHEN 'zepp_gpx' THEN
      -- Corrigido: usa colunas por amostra da tabela public.zepp_gpx_activity_details
      WITH s AS (
        SELECT
          CASE
            WHEN d.speed_meters_per_second IS NOT NULL AND d.speed_meters_per_second > 0
    ...
    WHEN 'polar' THEN
      -- Mantém compatibilidade com payloads JSONB legados
      WITH s AS (
        SELECT
          CASE
            WHEN NULLIF(elem->>'pace_min_km','') IS NOT NULL THEN (elem->>'pace_min_km')::numeric
...
-- Create trigger function for HealthKit activities in activity_chart_data
...
AS $function$
BEGIN
  -- Only for HealthKit activities
  IF NEW.activity_source = 'healthkit' THEN
    PERFORM public.calculate_variation_analysis(NEW.user_id, 'healthkit', NEW.activity_id);
  END IF;
  RETURN NEW;
END;
$function$;

-- Create trigger on activity_chart_data for HealthKit activities
DROP TRIGGER IF EXISTS trg_variation_analysis_healthkit ON public.activity_chart_data;
CREATE TRIGGER trg_variation_analysis_healthkit
  AFTER INSERT OR UPDATE ON public.activity_chart_data
  FOR EACH ROW
  EXECUTE FUNCTION public.trg_variation_analysis_recalc_healthkit();

-- Backfill existing HealthKit activities with chart data
DO $$
DECLARE
  activity_record RECORD;
  processed_count INTEGER := 0;
BEGIN
  -- Loop through all HealthKit activities that have chart data
  FOR activity_record IN 
    SELECT DISTINCT user_id, activity_id
    FROM public.activity_chart_data 
    WHERE activity_source = 'healthkit'
      AND jsonb_array_length(series_data) > 0
  LOOP
    -- Calculate variation analysis for each activity
    PERFORM public.calculate_variation_analysis(
  ...
  RAISE NOTICE 'Processed % HealthKit activities for variation analysis', processed_count;
END $$;