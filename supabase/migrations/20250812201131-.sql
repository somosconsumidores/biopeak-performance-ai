...
-- Try to unschedule existing job if present (idempotent)
DO $$
BEGIN
  PERFORM cron.unschedule('scheduled-strava-token-renewal');
EXCEPTION WHEN OTHERS THEN
  -- ignore if job doesn't exist yet
  NULL;
END $$;

-- Create the cron job (runs at minute 0 and 50 of every hour => ~every 50 minutes)
