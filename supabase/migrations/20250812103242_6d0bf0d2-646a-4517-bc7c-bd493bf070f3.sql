...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'strava_webhook_logs' AND policyname = 'Service role can manage all webhook logs'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'strava_webhook_logs' AND policyname = 'Users can view their own webhook logs'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'strava_tokens' AND column_name = 'athlete_id') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_strava_webhook_logs_updated_at'
  ) THEN
