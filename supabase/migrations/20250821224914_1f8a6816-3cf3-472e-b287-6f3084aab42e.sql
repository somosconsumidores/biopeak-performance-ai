...
DROP POLICY IF EXISTS "Service role can manage variation analysis" ON public.variation_analysis;
...
DROP POLICY IF EXISTS "Users can view their own variation analysis" ON public.variation_analysis;
...
DROP POLICY IF EXISTS "Users can insert their own variation analysis" ON public.variation_analysis;
...
DROP POLICY IF EXISTS "Users can update their own variation analysis" ON public.variation_analysis;
...
DROP POLICY IF EXISTS "Users can delete their own variation analysis" ON public.variation_analysis;
...
BEGIN
  IF _activity_source = 'garmin' THEN
    SELECT array_agg((gad.heart_rate)::double precision) FILTER (WHERE gad.heart_rate IS NOT NULL),
           array_agg((CASE WHEN gad.speed_meters_per_second IS NOT NULL AND gad.speed_meters_per_second > 0 THEN (1000.0 / gad.speed_meters_per_second) / 60.0 ELSE NULL END))
    INTO v_hr, v_pace
    ...
    BEGIN
      ...
      FROM (
        SELECT NULLIF((sad.samples->>'heartrate')::numeric, NULL)::double precision AS hr,
               CASE WHEN NULLIF((sad.samples->>'velocity_smooth')::numeric, NULL) IS NOT NULL AND NULLIF((sad.samples->>'velocity_smooth')::numeric, NULL) > 0
                 THEN (1000.0 / NULLIF((sad.samples->>'velocity_smooth')::numeric, NULL)) / 60.0 ELSE NULL END::double precision AS pmin
        FROM public.strava_activity_details sad
        WHERE sad.user_id = _user_id AND sad.strava_activity_id::text = _activity_id
      ) s;
    EXCEPTION WHEN undefined_table THEN
      -- strava details table not present; ignore
      NULL;
    END;
  ...
  IF v_has_hr THEN
    ...
    IF v_avg IS NOT NULL AND v_avg > 0 AND v_std IS NOT NULL THEN
      v_cv := v_std / v_avg; v_hr_cv := ROUND((v_cv::numeric), 4); v_samples_hr := COALESCE(array_length(v_hr,1),0);
      v_hr_cat := CASE WHEN v_hr_cv IS NULL THEN NULL WHEN v_hr_cv < 0.05 THEN 'muito baixo' WHEN v_hr_cv < 0.10 THEN 'baixo' WHEN v_hr_cv < 0.15 THEN 'moderado' WHEN v_hr_cv < 0.20 THEN 'alto' ELSE 'muito alto' END;
    END IF;
  END IF;
  ...
  IF v_has_speed THEN
    ...
    IF v_avg IS NOT NULL AND v_avg > 0 AND v_std IS NOT NULL THEN
      v_cv := v_std / v_avg; v_pace_cv := ROUND((v_cv::numeric), 4); v_samples_pace := COALESCE(array_length(v_pace,1),0);
      v_pace_cat := CASE WHEN v_pace_cv IS NULL THEN NULL WHEN v_pace_cv < 0.05 THEN 'muito baixo' WHEN v_pace_cv < 0.10 THEN 'baixo' WHEN v_pace_cv < 0.15 THEN 'moderado' WHEN v_pace_cv < 0.20 THEN 'alto' ELSE 'muito alto' END;
    END IF;
  END IF;

  v_diag := CASE
    WHEN v_hr_cv IS NOT NULL AND v_pace_cv IS NOT NULL THEN
      CASE WHEN v_pace_cv <= 0.10 AND v_hr_cv <= 0.10 THEN 'Ritmo e esforço muito consistentes – ótima execução.'
           ...
           ELSE 'Variação considerável em ritmo e esforço – ajuste de estratégia recomendado.' END
    ...
    ELSE 'Dados insuficientes para análise.' END;
...
-- 5) Triggers (guard each in DO to avoid failures if table missing)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='garmin_activity_details') THEN
    CREATE OR REPLACE FUNCTION public.trg_variation_garmin()
    RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
    BEGIN
      PERFORM public.calculate_variation_analysis(NEW.user_id, 'garmin', NEW.activity_id);
      RETURN NEW;
    END;$$;
    DROP TRIGGER IF EXISTS trg_variation_garmin ON public.garmin_activity_details;
    CREATE TRIGGER trg_variation_garmin AFTER INSERT ON public.garmin_activity_details
    FOR EACH ROW EXECUTE FUNCTION public.trg_variation_garmin();
  END IF;
END $$;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='strava_activity_details') THEN
    CREATE OR REPLACE FUNCTION public.trg_variation_strava()
    RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
    BEGIN
      PERFORM public.calculate_variation_analysis(NEW.user_id, 'strava', NEW.strava_activity_id::text);
      RETURN NEW;
    END;$$;
    DROP TRIGGER IF EXISTS trg_variation_strava ON public.strava_activity_details;
    CREATE TRIGGER trg_variation_strava AFTER INSERT ON public.strava_activity_details
    FOR EACH ROW EXECUTE FUNCTION public.trg_variation_strava();
  END IF;
END $$;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='strava_gpx_activity_details') THEN
    CREATE OR REPLACE FUNCTION public.trg_variation_strava_gpx()
    RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
    BEGIN
      PERFORM public.calculate_variation_analysis(NEW.user_id, 'strava_gpx', NEW.activity_id);
      RETURN NEW;
    END;$$;
    DROP TRIGGER IF EXISTS trg_variation_strava_gpx ON public.strava_gpx_activity_details;
    CREATE TRIGGER trg_variation_strava_gpx AFTER INSERT ON public.strava_gpx_activity_details
    FOR EACH ROW EXECUTE FUNCTION public.trg_variation_strava_gpx();
  END IF;
END $$;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='polar_activity_details') THEN
    CREATE OR REPLACE FUNCTION public.trg_variation_polar()
    RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
    BEGIN
      PERFORM public.calculate_variation_analysis(NEW.user_id, 'polar', NEW.activity_id);
      RETURN NEW;
    END;$$;
    DROP TRIGGER IF EXISTS trg_variation_polar ON public.polar_activity_details;
    CREATE TRIGGER trg_variation_polar AFTER INSERT ON public.polar_activity_details
    FOR EACH ROW EXECUTE FUNCTION public.trg_variation_polar();
  END IF;
END $$;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='zepp_gpx_activity_details') THEN
    CREATE OR REPLACE FUNCTION public.trg_variation_zepp_gpx()
    RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
    BEGIN
      PERFORM public.calculate_variation_analysis(NEW.user_id, 'zepp_gpx', NEW.activity_id);
      RETURN NEW;
    END;$$;
    DROP TRIGGER IF EXISTS trg_variation_zepp_gpx ON public.zepp_gpx_activity_details;
    CREATE TRIGGER trg_variation_zepp_gpx AFTER INSERT ON public.zepp_gpx_activity_details
    FOR EACH ROW EXECUTE FUNCTION public.trg_variation_zepp_gpx();
  END IF;
END $$;

-- 6) Backfill only for Garmin to avoid missing-table issues
DO $$
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='garmin_activity_details') THEN
    FOR r IN (
      ...
      LEFT JOIN public.variation_analysis va
        ON va.user_id = gad.user_id AND va.activity_id = gad.activity_id AND va.activity_source = 'garmin'
      WHERE va.id IS NULL
      LIMIT 500
    ) LOOP
