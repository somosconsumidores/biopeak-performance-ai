...
export const useRealtimeSession = () => {
  ...
  const completeSession = useCallback(async (subjectiveFeedback?: any) => {
    ...
    try {
      ...
      // Save GPS coordinates for heatmap
      if (gpsCoordinatesRef.current.length > 0) {
        console.log('🗺️ Saving GPS coordinates for heatmap...');
        const { error: coordsError } = await supabase
          ...
          .insert({
            ...
            starting_latitude: gpsCoordinatesRef.current[0][0],
            starting_longitude: gpsCoordinatesRef.current[0][1]
          });
        
        if (coordsError) {
          console.error('⚠️ Error saving GPS coordinates:', coordsError);
        } else {
          console.log('✅ GPS coordinates saved successfully');
        }
      }

      // Build chart data from performance snapshots for pace histogram
      console.log('📊 Building chart data for pace analysis...');
      try {
        ...
        if (snapshotsError) {
          console.error('⚠️ Error fetching snapshots:', snapshotsError);
        } else if (snapshots && snapshots.length > 0) {
          // Transform snapshots into chart data format
          const chartData = snapshots.map(snapshot => ({
            distance_km: snapshot.snapshot_at_distance_meters / 1000,
            elapsed_seconds: snapshot.snapshot_at_duration_seconds,
            pace_min_km: snapshot.current_pace_min_km,
            speed_ms: snapshot.current_speed_ms,
            heart_rate_bpm: snapshot.current_heart_rate,
            elevation_m: snapshot.elevation_meters,
          ...
          const { error: chartError } = await supabase
            ...
            .insert({
              ...
              total_distance_km: sessionData.currentDistance / 1000,
              total_duration_seconds: sessionData.currentDuration,
              average_pace_min_km: sessionData.averagePace
            });

          if (chartError) {
            console.error('⚠️ Error saving chart data:', chartError);
          } else {
            console.log('✅ Chart data saved successfully');
          }
        }
      } catch (chartErr) {
        console.error('⚠️ Error processing chart data:', chartErr);
      }

      // Calculate best 1km segments
      console.log('🏃 Calculating best 1km segments...');
      try {
        const { error: segmentsError } = await supabase.functions.invoke('calculate-best-1km-segments', {
          body: {
            activity_id: sessionData.sessionId,
            user_id: user?.id,
            activity_source: 'biopeak_app'
        ...
        if (segmentsError) {
          console.error('⚠️ Error calculating segments:', segmentsError);
        } else {
          console.log('✅ Best 1km segments calculated');
        }
      } catch (segErr) {
        console.error('⚠️ Error invoking segments calculation:', segErr);
      }

      // Calculate statistics metrics
      console.log('📈 Calculating statistics metrics...');
      try {
        const { error: statsError } = await supabase.functions.invoke('calculate-statistics-metrics', {
          body: {
            activity_id: sessionData.sessionId,
            user_id: user?.id,
            activity_source: 'biopeak_app'
        ...
        if (statsError) {
          console.error('⚠️ Error calculating statistics:', statsError);
        } else {
          console.log('✅ Statistics metrics calculated');
        }
      } catch (statsErr) {
        console.error('⚠️ Error invoking statistics calculation:', statsErr);
      }
