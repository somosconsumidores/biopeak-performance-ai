...
export const StravaPaceChart = ({ stravaActivityId }: StravaPaceChartProps) => {
  ...
  // Calculate average pace for reference line (excluding null values)
      ...
      <CardContent>
        <div className={`${isMobile ? 'h-64' : 'h-72 sm:h-80'}`}>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              {/* Reference line for average pace - hide on mobile for clarity */}
              {avgPace > 0 && !isMobile && (
                <ReferenceLine 
                  y={avgPace} 
                  stroke="hsl(var(--primary))" 
        ...
        <div className={`mt-4 grid gap-2 ${isMobile ? 'grid-cols-1 text-xs' : 'grid-cols-1 sm:grid-cols-2 text-xs sm:text-sm'}`}>
          <div className="flex items-center space-x-2 justify-center sm:justify-start">
            ...
            <span className="truncate">Ritmo Médio: {avgPace > 0 ? `${Math.floor(avgPace)}:${Math.round((avgPace - Math.floor(avgPace)) * 60).toString().padStart(2, '0')}/km` : 'N/A'}</span>
          </div>
