...
interface ZeppGpxActivitySummary {
  id: string;
  activity_id: string;
  name: string | null;
...
export const ZeppGpxImport: React.FC = () => {
  const { user } = useAuth();
  ...
  const [recent, setRecent] = useState<ZeppGpxActivitySummary[]>([]);
  ...
  const handleUpload = async () => {
    try {
      ...
      const { data: res, error: fnErr } = await supabase.functions.invoke('import-zepp-gpx', {
      ...
      console.error('Zepp GPX import error', e);
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Envie um arquivo GPX 1.1 da Zepp. As atividades serão identificadas como "Zepp GPX".
            </CardDescription>
          </div>
        </div>
      </CardHeader>
              ...
              {recent.map((a) => (
                <div key={a.id} className="metric-card flex items-center justify-between">
                  <div className="space-y-1">
                    <div className="font-medium text-sm">{a.name || 'Atividade Zepp GPX'}</div>
